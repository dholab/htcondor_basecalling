...
print_usage() {
	...
	echo "  --dorado: The name of the dorado tar archive, which itself contains the prebuilt 'dorado-0.8.0-linux-x64' directory and a pre-downloaded 'models/' directory (defaults to dorado.tar.gz)."
}
...
tar xf $dorado &&
	export PATH=$PATH:$(pwd)/dorado/models:$(pwd)/dorado/dorado-0.8.0-linux-x64/bin:$(pwd)/dorado/dorado-0.8.0-linux-x64/lib
if [ $? -eq 0 ]; then
